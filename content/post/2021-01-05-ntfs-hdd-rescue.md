...
- 故障した 4TB より大きい別の HDD 2つ
  ...
  - 1つは故障HDDのバックアップイメージの保存、もう1つは作業用ディクスとして修復に使用
- Linux PC
...
`ddrescue` コマンドでディスクイメージ `*.img` にバックアップすることができたで、これをまた別の 4TB 以上の HDD に焼いて、故障したHDDのクローンディスクを作成します。
...
ディレクトリ構造が復元できなくても、バイナリからファイルを単位で探し出すソフトもあります。

復旧作業を行うときは、 `ddrescue` でバックアップを行ってからクローンディスクを作成して作業を行いましょう。

故障したディクスに対して直接いろいろ試すのは危険です。どんどん壊れていく可能性があります。
