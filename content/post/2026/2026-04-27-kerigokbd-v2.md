...
- ケースの間にクッションテープを挟んだガスケット風マウントで打鍵感を改善
...
KERIgoKBD のケースは、3Dプリントした複数の部品を組み合わせて作っています。そのまま組み立てると、キーを打ったときに部品同士がこすれ、ギシギシと音がすることがありました。

そこで v2 では、ケースのつなぎ目に **GRIPLUS のクッションテープ** を挟んでいます。構造としては、簡易的なガスケットマウントのような形です。

{{< postfig src="kerigokbd_v1.2_case_back.jpg" title="KERIgoKBD v2 の裏面全体" width="640px">}}

{{< postfig src="kerigokbd_v1.2_case_connection.jpg" title="ケースの間に挟んだ GRIPLUS クッションテープ" width="640px">}}

クッションテープを挟むだけの簡単な対策ですが、部品同士が直接こすれなくなり、気になっていた音は完全になくなりました。打鍵感もソフトになり、いい感じです。
